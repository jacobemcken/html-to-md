...
; Maybe avoid attr name overlap between HTML and internal (intermediate MD datastructure for lib)
; Create test cases
; Test case with code inside link or bold inside italic etc.
(comment
  ...
  (no-infinity (unfoldz '() ::block (render-elements {} b8)))
  )
...
(comment
  ...
  (def b8 [:a {:href "https://clojure.org"} "Some " [:code "link"]])
  1
  (->> (render-elements {} b8)
       (unfoldz '() {:before ::block :after ::block})
       #_#_arrange
...
;; Only apply spacing if both elements are "spacing elements" p, ul and blockqoute are... li isn't.